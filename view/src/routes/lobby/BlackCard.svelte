	...
	<div class="w-full overflow-x-auto px-4 py-6">
		<div class="flex w-full items-end space-x-12">
			<div class="relative z-30 pt-6">
				<Card
					card={black_card}
					card_classes="bg-surface-950 shadow-xl"
			...
			{#each revealed_cards as pair, i}
				...
				<div
					class="group flex flex-shrink-0 cursor-pointer items-center pt-6"
					onclick={() => selectCards(i)}
				>
					{#each pair as card}
						<Card
							{card}
							card_classes="bg-surface-50 first:ml-0 ml-[-18px] transition-transform
              			{i === selectedIndex
								? 'z-20 -translate-y-4 scale-110'
